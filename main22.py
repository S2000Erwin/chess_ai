...
# Lesson 22: Add FastAPI support for webapp. Add React frontend
...
class PiecesImage:
    ...
    def get_image(self, color, role):
        """
        get_image
        get image of a particular role and color fro resources
        """
...
class Chess:
    ...
    def clone(self):
        ...
        new_chess.full_move = self.full_move
        new_chess.half_move = self.half_move
        return new_chess
    ...
    def get_fen(self) -> str:
        """get_fen"""
        def make_pieces():
            def make_one_row(row):
                def translate_one_piece(piece) -> str:
                    role_mappings = {
                        'king':'k',
                        'queen':'q',
                        'rook':'r',
                    ...
                    if isinstance(piece, Piece):
                        ...
                        if role in role_mappings:
                            r = role_mappings[role]
                            if piece.color == 'white':
                                r = r.upper()
                            return r
                    return '1'
                pieces = [translate_one_piece(self.get_piece((row, column))) for column in range(8)]
                return ''.join(pieces)
            return '/'.join([make_one_row(row) for row in range(8)])
        fen_pieces = make_pieces()
        fen_color = 'w' if self.player == 'white' else 'b'
        black_rook = self.get_piece((0,0))
        black_king = self.get_piece((0,3))
        white_rook = self.get_piece((7,0))
        white_king = self.get_piece((7,3))
        fen_castle = ''
        if isinstance(white_king, King) and not white_king.moved and \
            isinstance(white_rook, Rook) and not white_rook.moved:
            fen_castle = ''.join([fen_castle, 'K'])
        if isinstance(black_king, King) and not black_king.moved and \
            isinstance(black_rook, Rook) and not black_rook.moved:
            fen_castle = ''.join([fen_castle, 'k'])
        if fen_castle == '':
            fen_castle = '-'
        fen_en_passant = '-'
        if self.en_passant:
            row = str(8 - self.en_passant[0])
            col = str(chr(ord('a') + self.en_passant[1]))
            fen_en_passant = col + row
        fen_half_move = str(self.half_move)
        fen_full_move = str(self.full_move + 1)
        return ' '.join([fen_pieces, fen_color, fen_castle, fen_en_passant,
                         fen_half_move, fen_full_move])
                            ...
                            if right:
                                ...
                                if self.state == 'free':
                                    ...
                                    if grid_pos in self.chess.get_legal_moves(self.source):
                                        piece = self.chess.get_piece(self.source)
                                        if isinstance(piece, Pawn) and (grid_pos[0] == 0 or grid_pos[0] == 7):
                                            answer = 0
                                        ...
                                        if isinstance(piece, Piece):
                                            print(f'{piece.color}: {notation((self.source, grid_pos))}')
...
def fen_to_grid_pos(fen:str) -> Tuple[int, int]:
    col = ord(fen[0]) - ord('a')
    row = 8 - int(fen[1])
    return row, col

def grid_pos_to_fen(grid_pos) -> str:
    row = str(8 - grid_pos[0])
    col = str(chr(ord('a') + grid_pos[1]))
    return ''.join([col,row])
    ...
    def _setup_routes(self):
        ...
        async def status():
            return {
                ...
                'active': self.chess.winner is None,
                'winner': self.chess.winner
            }

        @self.router.get('/valid_moves')
        async def valid_moves(square:str):
            moves = self.chess.get_legal_moves(fen_to_grid_pos(square))
            moves = [grid_pos_to_fen(move) for move in moves]
            return {'moves': moves}
        ...
        async def move(req:Request):
            result = False
            body = await req.json()
            source = fen_to_grid_pos(body.get('source'))
            target = fen_to_grid_pos(body.get('target'))
            if target in self.chess.get_legal_moves(source):
                self.chess.apply_move(source, target)
                # let the bot move
                current_player = next(filter(lambda x: x.color == self.chess.player, self.players))
...
def web_app():
    """web_app"""
    app = WebApp()
    uvicorn.run(app.fastapi, host='127.0.0.1', port=8083)
